...
const SignUpPage = () => {
  ...
  const validateForm = ()=> {
    ...
    if(!/\S+@\S+\.\S+/.test(formData.email)){ toast.error("Invalid email formate"); return false;}
    if(!formData.password){ toast.error("Password is required"); return false;}
    if(formData.password.length < 6){ toast.error("Password must be at least 6 cahracters"); return false;}

    return true
  };

  //function for prevent reloading
